...
Markdown for the analysis of the control section.
...
I am personally sorry for this inconvenience. A more robust and reproducible pipeline is in the process of making. Thank you for your understanding.


Files required to run the script: 
- original files: "orig/seurat_2ndSpatial.rds", "Metanalysis/ABA regional markers/genes/"
- intermediate, created files: "data/seurat_ctrl.Rds"
...
Miscellaneous. Font, color scheme...
```{r misc}
...
Creating a subfolder for the results.
```{r file folder loc}
if(!dir.exists("results/SpatialFigures/")){dir.create("results/SpatialFigures/")}
if(!dir.exists("results/SpatialFigures/Figure1")){dir.create("results/SpatialFigures/Figure1"); ws <- "results/SpatialFigures/Figure1"}else{ws <- "results/SpatialFigures/Figure1"}
```
...
ABAregions <- ABAfiles  %>% str_replace(pattern = "ABAmarkers_", replacement = "ABA_") %>% str_remove(pattern = ".txt")
SeuratRegions <- seurat_ctrl$BrainRegions %>% levels()
