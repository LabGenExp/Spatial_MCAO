...
```{r libraries}
library(Seurat); library(SeuratWrappers); library(dplyr); library(ggplot2); library(tibble); library(openxlsx); library(RColorBrewer); library(magrittr); library(gridExtra); library(patchwork); library(plotly); library(dittoSeq); library(pheatmap)
...
p <- DotPlot(astro.seurat, assay = "SCT", features = features_test, group.by = "cell_type_2", cols = c("#FFF7FB", "#014636")) + 
  theme(text = element_text(family = "OpenSans", size = 20), 
        ...
        axis.text.x = element_text(angle = 55, vjust = 1, hjust = 1, size = 25), 
        axis.ticks = element_blank(), legend.position = "right") + ylab("") + xlab("");p
p %>% ggsave(filename = paste0(ws, "/DotPlot_sn_Astroepe.tiff"), device = 'tiff', dpi = 300, width = 7.5, height = 3)  
}
```
...
## Dotplot ####
{
  ...
  p <- DotPlot(mg.seurat, assay = "SCT", features = features_test, group.by = "cell_type_2", cols = c("#FFF7FB", "#A50F15")) + 
    theme(text = element_text(family = "OpenSans", size = 35),
        axis.text.y = element_text(size = 50, hjust = 1), 
        axis.text.x = element_text(angle = 55, vjust = 1, hjust = 1, size = 35), 
        axis.ticks = element_blank(), legend.position = "right") + ylab("") + xlab("");p
  p %>% ggsave(filename = paste0(ws, "/DotPlot_sn_MG.tiff"), device = 'tiff', dpi = 300, width = 6.5, height = 3)
}
