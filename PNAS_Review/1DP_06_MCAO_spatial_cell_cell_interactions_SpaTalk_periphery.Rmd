...
When finished, load periphery Spatalk objects
...
mtx <- spatial.seurat %>% 
  ...
  mutate(Apoe_Trem2_lr_exp = case_when(
    ...
    TRUE ~ mean(c(exp_Apoe, exp_Trem2))
  )) %>% 
  ungroup()
...
SpaTalk::plot_lrpair(spatalk.list[["Ctrl"]], 
                     celltype_sender = "OLs", 
...
result.list[["Apoe_Trem2_downstream_targets"]] <- sections %>% lapply(\(x){
  # get the LR pairs
  plot <- SpaTalk::plot_lr_path(
  ...
  downstream_targets <- plot[["plot_env"]][["plot_node_new"]][["gene"]] %>% unique
...
result.list[["GO_apoe_trem_downstream_targets"]] <- clusterProfiler::enrichGO(
...
result.list[["GO_apoe_trem_downstream_df"]] <- downstream_go %>% 
  ...
  dplyr::mutate(Description = Description %>% 
                  ...
                  stringr::str_replace_all(pattern = "erk", replacement = "ERK"),
...
heatmap_data <- spatial.seurat@meta.data %>% 
  ...
  summarise(
    ...
    `ERK1 and ERK2 cascade` = mean(`GO_Positive regulation of erk1 and erk2 cascade3`), 
    Gliogenesis = mean(GO_Gliogenesis4), 
    `Glucose Metabolic Process` = mean(`GO_Glucose metabolic process5`),
    Chemotaxis = mean(GO_Chemotaxis6), 
    `Cytokine Production` = mean(`GO_Positive regulation of cytokine production7`), 
    `Tissue Remodeling` = mean(`GO_Tissue remodeling8`),
