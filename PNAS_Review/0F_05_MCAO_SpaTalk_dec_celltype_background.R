# OF_05_MCAO_SpaTalk_dec_celltype_background
...
start.time <- Sys.time()
message(paste0("Initiating the computations. The time is: ", start.time))
sections <- c("Ctrl", "1DPI", "3DPI", "7DPI")

# Deconvolution to single-cell resolution
if(file.exists(file.path("data", "SpaTalk_RefZeng2023_MCAO_spatial.Rds"))){
  cat(crayon::silver("Skipping computing single-cell data. The file already exists.", "\n"))
} else {
  #load the data
  load(file.path("data", "1DP_05_MCAO_dec_celltype_background.Rdata"))
  ...
  if (length(dec.spatalk.list) == length(sections)) {
    ...
    saveRDS(dec.spatalk.list, file = file.path("data", "SpaTalk_RefZeng2023_MCAO_spatial.Rds"))
...
if(file.exists(file.path("data", "SpaTalk_RefZeng2023_MCAO_spatial_lr.Rds"))){
  ...
  if(!exists("dec.spatalk.list")){
    dec.spatalk.list <- readRDS(file = file.path("data", "SpaTalk_RefZeng2023_MCAO_spatial.Rds"))
  ...
  # save the output
  saveRDS(dec.spatalk.list_lr, file = file.path("data", "SpaTalk_RefZeng2023_MCAO_spatial_lr.Rds"))
...
if(!exists("dec.spatalk.list_lr")){
  dec.spatalk.list_lr <- readRDS(file = file.path("data", "SpaTalk_RefZeng2023_MCAO_spatial_lr.Rds"))
}
...
for(section in seq_along(dec.spatalk.list_lr)){
  ...
  file_name <- paste0("SpaTalk_RefZeng2023_MCAO_spatial_full_", section_name, ".Rdata")
