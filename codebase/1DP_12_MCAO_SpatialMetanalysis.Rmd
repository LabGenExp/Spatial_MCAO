---
title: "1DP_12_MCAO_SpatialMetanalysis"
author: "DanielZucha"
date: "2024-05-01"
...
```{r separate seurat objects for each section}
# identify sections
...
seurat.list[["Sham1-1"]] %<>%
  rotateSeuratImage(slide = "Sham1.1",
                    rotation = "180")
...
seurat.list[["Sham1-2"]] %<>%
  rotateSeuratImage(slide = "Sham1.2",
                    rotation = "180") %>% 
  rotateSeuratImage(slide = "Sham1.2",
                    rotation = "Hf")
...
seurat.list[["Sham1-3"]] %<>%
  rotateSeuratImage(slide = "Sham1.3",
                    rotation = "180") %>% 
  rotateSeuratImage(slide = "Sham1.3",
                    rotation = "Hf")
...
seurat.list[["Sham1-4"]] %<>%
  rotateSeuratImage(slide = "Sham1.4",
                    rotation = "180") %>% 
  rotateSeuratImage(slide = "Sham1.4",
                    rotation = "Hf")
...
```{r combine into one spatial seurat}
...
```{r save the spatial Han 2024 seurat}
# save the seurat
...
# Simplified brain areas
spatial.seurat@meta.data$BrainRegionShort %>% levels()
spatial.seurat@meta.data %<>% 
  mutate(BrainArea = factor(case_when(
    BrainRegionShort == "AMY" ~ "Telencephalon", 
    BrainRegionShort == "CP" ~ "Telencephalon", 
    BrainRegionShort == "CS" ~ "Telencephalon", 
    BrainRegionShort == "CTX1-4" ~ "Cortex", 
    BrainRegionShort == "CTX5-6" ~ "Cortex", 
    BrainRegionShort == "FT" ~ "Telencephalon", 
    BrainRegionShort == "GLS" ~ "Cortex", 
    BrainRegionShort == "HIP" ~ "Telencephalon", 
    BrainRegionShort == "HY" ~ "Diencephalon", 
    BrainRegionShort == "ISD3c" ~ "LesionCore", 
    BrainRegionShort == "ISD3p" ~ "LesionPeriphery", 
    BrainRegionShort == "PIR" ~ "Telencephalon", 
    BrainRegionShort == "TH" ~ "Diencephalon", 
    BrainRegionShort == "V" ~ "Telencephalon" 
  ), levels = c("Diencephalon", "Telencephalon", "Cortex", "LesionCore", "LesionPeriphery")))
```
...
Rename sections based on time and bregma location
```{r rename images based on time and bregma location}
spatial.seurat@images %>% names
names(spatial.seurat@images) <- c("3DPI_Bregma_0.4", "3DPI_Bregma_-0.8", "3DPI_Bregma_-2.0", "3DPI_Bregma_-3.2", 
                               "Sham_Bregma_0.4", "Sham_Bregma_-0.8", "Sham_Bregma_-2.0", "Sham_Bregma_-3.2")

sections <- c("3DPI_Bregma_0.4", "3DPI_Bregma_-0.8", "3DPI_Bregma_-2.0", "3DPI_Bregma_-3.2", 
                               "Sham_Bregma_0.4", "Sham_Bregma_-0.8", "Sham_Bregma_-2.0", "Sham_Bregma_-3.2")

spatial.seurat@meta.data %<>% 
  mutate(
    Bregma_SectionName = factor(case_when(
      SectionName == "PT1-1" ~ "3DPI_Bregma_0.4", 
      SectionName == "PT1-2" ~ "3DPI_Bregma_-0.8", 
      SectionName == "PT1-3" ~ "3DPI_Bregma_-2.0", 
      SectionName == "PT1-4" ~ "3DPI_Bregma_-3.2", 
      SectionName == "Sham1-1" ~ "Sham_Bregma_0.4", 
      SectionName == "Sham1-2" ~ "Sham_Bregma_-0.8", 
      SectionName == "Sham1-3" ~ "Sham_Bregma_-2.0", 
      SectionName == "Sham1-4" ~ "Sham_Bregma_-3.2"
    ))
  )
...
Some images are rotated weirdly. Let's adjust as necessary
```{r Flip images as necessary}
# Sham image
seurat.list[["Sham"]] %>% 
  SpatialPlot(
    features = "nFeature_Spatial", 
    images = "Sham") ## needs R90 rotation
...
```{r combine seurat objects into one}
# merge into one large seurat
spatial.seurat <- merge(x = seurat.list[[1]], 
                        y = seurat.list[2:length(seurat.list)], 
                        add.cell.ids = names(seurat.list), 
...

{
  # Clustering analysis
  spatial.seurat %<>% FindNeighbors(reduction = "pca", dims = 1:nDims)
  spatial.seurat %<>% FindClusters(resolution = 1.2, cluster.name = "unintegrated_clusters")
  
  plot.list[["UMAP_noInt"]] <-
    spatial.seurat %>%
    DimPlot(
      reduction = "umap_unintegrated",
      group.by = "seurat_clusters",
  ...
  print(plot.list[["UMAP_noInt"]])
  
  plot.list[["SpatialPlot_noInt"]] <-
    spatial.seurat %>%
    SpatialPlot(
      group.by = "seurat_clusters",
      image.alpha = 0,
  ...
  print(plot.list[["SpatialPlot_noInt"]])
...
spatial.seurat@meta.data %<>% mutate("BrainRegion" = factor(case_when(
  ...
  seurat_clusters == 1 ~ "Isocortex L6", 
  seurat_clusters == 2 ~ "Caudoputamen", 
  seurat_clusters == 3 ~ "Hypothalamus",
  seurat_clusters == 4 ~ "Caudoputamen",
  seurat_clusters == 5 ~ "Hypothalamus", 
  seurat_clusters == 6 ~ "Piriform area", 
  seurat_clusters == 7 ~ "Thalamus",
  seurat_clusters == 8 ~ "Isocortex L5", 
  seurat_clusters == 9 ~ "Isocortex L5", 
  seurat_clusters == 10 ~ "Ischemic area D3 periphery", 
  seurat_clusters == 11 ~ "Fiber tracts", 
  seurat_clusters == 12 ~ "Amygdalar area", 
  seurat_clusters == 13 ~ "Isocortex L1-4", 
  seurat_clusters == 14 ~ "GLS", 
  seurat_clusters == 15 ~ "Isocortex L1-4", 
  seurat_clusters == 16 ~ "Cortical subplate", 
  seurat_clusters == 17 ~ "Ischemic area D7 periphery", 
  seurat_clusters == 18 ~ "Isocortex L1-4", 
  seurat_clusters == 19 ~ "Ischemic area D3 center", 
  seurat_clusters == 20 ~ "Caudoputamen",
  seurat_clusters == 21 ~ "Ventricle",
  seurat_clusters == 22 ~ "Fiber tracts",
  seurat_clusters == 23 ~ "Hypothalamus"
)))

spatial.seurat@meta.data %<>% mutate("BrainRegionShort" = factor(case_when(
  seurat_clusters == 0 ~ "CP", 
  seurat_clusters == 1 ~ "CTX6", 
  seurat_clusters == 2 ~ "CP", 
  seurat_clusters == 3 ~ "HY", 
  seurat_clusters == 4 ~ "CP",
  seurat_clusters == 5 ~ "HY", 
  seurat_clusters == 6 ~ "PIR", 
  seurat_clusters == 7 ~ "TH",
  seurat_clusters == 8 ~ "CTX5", 
  seurat_clusters == 9 ~ "CTX5", 
  seurat_clusters == 10 ~ "ISD3p", 
  seurat_clusters == 11 ~ "FT", 
  seurat_clusters == 12 ~ "AMY", 
  seurat_clusters == 13 ~ "CTX1-4", 
  seurat_clusters == 14 ~ "GLS", 
  seurat_clusters == 15 ~ "CTX1-4", 
  seurat_clusters == 16 ~ "CS", 
  seurat_clusters == 17 ~ "ISD7p", 
  seurat_clusters == 18 ~ "CTX1-4", 
  seurat_clusters == 19 ~ "ISD3c", 
  seurat_clusters == 20 ~ "CP", 
  seurat_clusters == 21 ~ "V",
  seurat_clusters == 22 ~ "FT",
  seurat_clusters == 23 ~ "HY"
)))
...
spatial.seurat@meta.data %<>% 
  mutate(BrainArea = factor(case_when(
    ...
    BrainRegionShort == "CS" ~ "Telencephalon", 
    BrainRegionShort == "CTX1-4" ~ "Cortex", 
    BrainRegionShort == "CTX5" ~ "Cortex", 
    BrainRegionShort == "CTX6" ~ "Cortex", 
    BrainRegionShort == "FT" ~ "Telencephalon", 
    BrainRegionShort == "GLS" ~ "Cortex", 
    BrainRegionShort == "HIP" ~ "Telencephalon", 
    BrainRegionShort == "HY" ~ "Diencephalon", 
    BrainRegionShort == "ISD3c" ~ "LesionCore", 
    BrainRegionShort == "ISD3p" ~ "LesionPeriphery", 
    BrainRegionShort == "ISD7p" ~ "LesionPeriphery", 
    BrainRegionShort == "PIR" ~ "Telencephalon", 
    BrainRegionShort == "TH" ~ "Diencephalon", 
    BrainRegionShort == "V" ~ "Telencephalon" 
  ), levels = c("Diencephalon", "Telencephalon", "Cortex", "LesionCore", "LesionPeriphery")))

spatial.seurat@meta.data$BrainArea %>% table
```
...
rename images
```{r rename scott section images}
spatial.seurat@images %>% names
names(spatial.seurat@images) <- c("10DPI", "21DPI", "2DPI", "Sham")
sections <- c("Sham", "2DPI", "10DPI", "21DPI")
```
